...
class BinaryCounter:
    ...
    def gamma_rate(self):
        ...
        for pos in range(self.line_len):
            fin += self.max_occurences(self.data, pos)
        return int(fin, 2)

    def epsilon_rate(self):
        fin = ''
        for pos in range(self.line_len):
            fin += self.min_occurences(self.data, pos)
        return int(fin, 2)
    ...
    def oxygen_rate(self):
        ...
        for pos in range(self.line_len):
            if len(ox_rate) == 1:
                ...
                ox_rate = pd.DataFrame([i for i in ox_rate if i[pos] == self.max_occurences(ox_rate, pos)])[0]
    ...
    def co2_rate(self):
        ...
        for pos in range(self.line_len):
            if len(co2_rate) == 1:
                ...
                co2_rate = pd.DataFrame([i for i in co2_rate if i[pos] == self.min_occurences(co2_rate, pos)])[0]
